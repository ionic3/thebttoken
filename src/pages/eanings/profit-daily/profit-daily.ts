...
@IonicPage()
@Component({
  selector: 'page-profit-daily',
  templateUrl: 'profit-daily.html',
})
export class ProfitDailyPage {
	customer_id : any;
	count_history : any;
	history : any;
	history_load_no : any;
	...
	ionViewDidLoad() {
		...
		this.AccountServer.GetHisroryProfit(this.customer_id,'Profit day',0,10)
        .subscribe((data) => {
        	loading.dismiss();
			if (data)
			{
	...
	ionViewWillEnter() {
  	...
  	doInfinite(infiniteScroll : InfiniteScroll) {
		...
		this.AccountServer.GetHisroryProfit(this.customer_id,'Profit day',this.history.length,5)
        .subscribe((data) => {
        	if (data.length > 0)
			{
