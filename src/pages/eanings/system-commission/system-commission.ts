...
@IonicPage()
@Component({
  selector: 'page-system-commission',
  templateUrl: 'system-commission.html',
})
export class SystemCommissionPage {
	customer_id : any;
	count_history : any;
	history : any;
	history_load_no : any;
	...
	ionViewDidLoad() {
		...
		this.AccountServer.GetHisroryProfit(this.customer_id,'System commission',0,10)
        .subscribe((data) => {
        	loading.dismiss();
			if (data)
			{
	...
	ionViewWillEnter() {
  	...
  	doInfinite(infiniteScroll : InfiniteScroll) {
		...
		this.AccountServer.GetHisroryProfit(this.customer_id,'System commission',this.history.length,5)
        .subscribe((data) => {
        	if (data.length > 0)
			{
